...
describe('S3Service', () => {
  ...
  describe('createPresignedPut', () => {
    const mockSignedUrl = 'https://s3.test.com/test-bucket/files/test-file.pdf?signature=test';
    const mockFileUrl = 'https://s3.test.com/test-bucket/files/test-file.pdf';
    ...
    it('should create presigned URL successfully', async () => {
      const result = await service.createPresignedPut('test-file.pdf', 'application/pdf');

      expect(result).toEqual({
        uploadUrl: mockSignedUrl,
        fileUrl: expect.stringMatching(/^https:\/\/s3\.test\.com\/test-bucket\/files\/[a-f0-9-]+\.pdf$/),
    ...
    it('should create presigned URL without filename', async () => {
      ...
      expect(result).toEqual({
        uploadUrl: mockSignedUrl,
        fileUrl: expect.stringMatching(/^https:\/\/s3\.test\.com\/test-bucket\/files\/[a-f0-9-]+$/),
    ...
    it('should throw error for filename too long', async () => {
      ...
      await expect(service.createPresignedPut(longFilename, 'application/pdf'))
        .rejects.toThrow(BadRequestException);
    });

    it('should throw error for invalid content type', async () => {
      await expect(service.createPresignedPut('test.exe', 'application/x-executable'))
        .rejects.toThrow(BadRequestException);
    });
    ...
    it('should allow valid content types', async () => {
      ...
      for (const contentType of validTypes) {
        await expect(service.createPresignedPut('test.file', contentType))
    ...
    it('should handle AWS SDK errors', async () => {
      ...
      await expect(service.createPresignedPut('test.pdf', 'application/pdf'))
        .rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('constructor', () => {
    it('should throw error when required environment variables are missing', () => {
      ...
      expect(() => new S3Service()).toThrow('Missing required environment variables');
    });
  });
});
