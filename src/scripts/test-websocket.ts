...
interface UserPresenceEventData {
	userId: string;
	chatRoomId: string;
}
...
class WebSocketTester {
	...
	private setupUserListeners(user: TestUser) {
		...
		// Chat room events
		socket.on('joinedChatRoom', (data: ChatRoomEventData) => {
			console.log(`📝 ${name} joined room:`, data.chatRoomId);
		});

		socket.on('leftChatRoom', (data: ChatRoomEventData) => {
			console.log(`👋 ${name} left room:`, data.chatRoomId);
		});

		socket.on('chatRoomCreated', (data: ChatRoomCreatedData) => {
			console.log(`🏠 ${name} created room:`, data.id, data.name);
		});

		socket.on('chatRoomUpdated', (data: ChatRoomEventData) => {
			console.log(`✏️ ${name} updated room:`, data.chatRoomId);
		});

		socket.on('participantsAdded', (data: ChatRoomEventData) => {
			console.log(
				`➕ ${name} added participants to room:`,
				data.chatRoomId,
			);
		});

		socket.on('participantRemoved', (data: ChatRoomEventData) => {
			console.log(
				`➖ ${name} removed participant from room:`,
				data.chatRoomId,
			);
		});

		// Message events
		socket.on('newMessage', (data: MessageEventData) => {
			console.log(
				`💬 ${name} received message in room ${data.chatRoomId}:`,
				data.message.content,
			);
		});

		socket.on('messageSent', (data: MessageSentData) => {
		...
		// Typing events
		socket.on('userTyping', (data: TypingEventData) => {
			const status = data.isTyping ? 'typing' : 'stopped typing';
			console.log(
				`⌨️ User ${data.userId} ${status} in room ${data.chatRoomId}`,
			);
		});

		// User presence events
		socket.on('userJoined', (data: UserPresenceEventData) => {
			console.log(
				`👤 User ${data.userId} joined room ${data.chatRoomId}`,
			);
		});

		socket.on('userLeft', (data: UserPresenceEventData) => {
			console.log(`👤 User ${data.userId} left room ${data.chatRoomId}`);
		});

		// Error handling
		socket.on('error', (error: ErrorEventData) => {
	...
	async runTestScenario() {
		...
		try {
			...
			await this.testCreateChatRoom(user1, 'Test Load Discussion', [
				user2.id,
				user3.id,
			]);
			...
			await this.testSendMessage(
				user1,
				chatRoomId,
				'Hello team! How is the delivery going?',
			);
			await this.testSendMessage(
				user2,
				chatRoomId,
