	...
	try {
		...
		const messages = await Promise.all([
			...
			prisma.message.create({
				data: {
					...
					senderId: users[1].id,
					content:
						'I want more detailed information about the project timeline.',
					createdAt: new Date(Date.now() - 2 * 60 * 60 * 1000), // 2 hours ago
				},
			}),
			prisma.message.create({
			...
			prisma.message.create({
				data: {
					...
					senderId: users[2].id,
					content:
						'Fleet status update: All vehicles are operational.',
			...
			prisma.message.create({
				data: {
					...
					senderId: users[1].id,
					content:
						'New delivery route assigned. Please confirm receipt.',
		...
		await prisma.message.create({
			data: {
				...
				fileUrl:
					'https://s3.eu-central-1.wasabisys.com/tms-chat/files/87d89025-0be3-4f9b-8a88-084061d04178.txt',
		...
		console.log(
			`Created ${users.length} users, ${chatRooms.length} chat rooms, and ${messages.length + 1} messages`,
		);
