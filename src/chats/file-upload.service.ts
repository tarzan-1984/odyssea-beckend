...
export class FileUploadService {
	...
	constructor(private configService: ConfigService) {
		this.uploadProvider = this.configService.get(
	...
	async uploadFile(
		...
		try {
			...
			throw new BadRequestException(
				`File upload failed: ${errorWithResponse.message}`,
			);
		}
	}
	...
	private uploadToGoogleDrive(
		...
		const googleDriveApiKey = this.configService.get<string>(
			'GOOGLE_DRIVE_API_KEY',
		);
		const googleDriveFolderId = this.configService.get<string>(
	...
	private uploadToAzureBlob(
		...
		const azureConnectionString = this.configService.get<string>(
			'AZURE_STORAGE_CONNECTION_STRING',
		);
		const azureContainerName = this.configService.get<string>(
	...
	private uploadToLocal(
		...
		const _uploadPath = this.configService.get<string>(
			'UPLOAD_PATH',
			'./uploads',
		);
	...
	validateFile(file: Express.Multer.File): boolean {
		...
		// Check file type
		const isAllowedType = allowedMimeTypesArray.some((type: string) => {
			if (type.endsWith('/*')) {
