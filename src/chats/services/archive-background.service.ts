...
export class ArchiveBackgroundService {
	...
	async startFullArchiveAndDelete(chatRoomId: string) {
		...
		this.logger.log(
			`🚀 Starting full archive and delete for chat room: ${chatRoomId} (Job ID: ${jobId})`,
		);
		this.jobs.set(jobId, {
			id: jobId,
			status: 'processing',
		...
		(async () => {
			...
			try {
				this.logger.log(
					`📋 Beginning archive process for chat room: ${chatRoomId}`,
				);
				let totalArchived = 0;
				...
				while (true) {
					this.logger.log(
						`🔍 Looking for latest message in chat room: ${chatRoomId}`,
					);
					...
					if (!latest) {
						this.logger.log(
							`✅ No more messages found in chat room: ${chatRoomId}. Proceeding to delete chat room.`,
						);
					...
					dayCount++;
					this.logger.log(
						`📅 Processing day ${dayCount}: ${year}-${month}-${day} (Job ID: ${jobId})`,
					);

					// Check if archive for this day already exists
					const exists =
						await this.messagesArchiveService.hasArchivedMessages(
							chatRoomId,
							year,
							month,
							day,
					...
					if (exists) {
						this.logger.log(
							`⏭️ Archive already exists for ${year}-${month}-${day}. Deleting messages from DB and skipping.`,
						);
						...
						this.logger.log(
							`🗑️ Deleted ${deletedCount.count} messages from DB for ${year}-${month}-${day}`,
						);
						continue;
					}
					...
					this.logger.log(
						`📥 Fetching messages for ${year}-${month}-${day}...`,
					);
					...
					this.logger.log(
						`📊 Found ${dayMessages.length} messages for ${year}-${month}-${day}`,
					);

					if (dayMessages.length === 0) {
						this.logger.log(
							`⚠️ No messages found for ${year}-${month}-${day}, continuing...`,
						);
					...
					this.logger.log(
						`📦 Creating archive for ${year}-${month}-${day} with ${dayMessages.length} messages...`,
					);
					...
					this.logger.log(
						`✅ Day ${dayCount} completed: ${dayResult.archived} messages archived for ${year}-${month}-${day} in ${dayResult.processedBatches} batches`,
					);
				...
				this.logger.log(
					`🎉 Archive process completed for chat room: ${chatRoomId}`,
				);
				this.logger.log(
					`📈 Final stats: ${totalArchived} total messages archived, ${dayCount} days processed, ${processedBatches} total batches`,
				);
				...
				this.logger.error(
					`❌ Archive process failed for chat room: ${chatRoomId}`,
					error,
				);
				job.status = 'failed';
	...
	async startArchive(
		...
		this.processArchiveInBackground(jobId, chatRoomId, batchSize).catch(
			(error) => {
				this.logger.error(
					`Background archive failed for job ${jobId}:`,
					error,
				);
				const job = this.jobs.get(jobId);
				if (job) {
		...
		this.logger.log(
			`Background archive started with ID: ${jobId}${chatRoomId ? ` for chat room ${chatRoomId}` : ' for all chat rooms'}`,
		);
	 ...
	 */
	private async processArchiveInBackground(
		jobId: string,
		chatRoomId?: string,
		batchSize: number = 50,
	): Promise<void> {
		...
		try {
			...
			for (const chatRoom of chatRooms) {
				...
				this.logger.log(
					`Processing chat room ${chatRoom.id} (${chatRoom._count.messages} old messages)`,
				);

				const roomResult = await this.processChatRoomArchive(
					jobId,
					chatRoom.id,
					cutoffDate,
					batchSize,
				...
				job.progress = Math.min(
					100,
					(chatRoomsProcessed / chatRooms.length) * 100,
				);

				this.logger.log(
					`Chat room ${chatRoom.id} completed: ${roomResult.archived} archived, ${roomResult.skipped} skipped`,
				);
			...
			this.logger.log(
				`Background archive completed for job ${jobId}: ${totalArchived} messages archived, ${totalSkipped} skipped, ${chatRoomsProcessed} chat rooms processed`,
			);
		} catch (error) {
			this.logger.error(
				`Background archive failed for job ${jobId}:`,
	 ...
	 */
	private async processChatRoomArchive(
		jobId: string,
		chatRoomId: string,
		cutoffDate: Date,
		batchSize: number,
		...
		for (const [dayKey, messages] of messagesByDay) {
			...
			this.logger.log(
				`Processing day ${job.currentDay} for chat room ${chatRoomId} (${messages.length} messages)`,
			);
			...
			if (archiveExists) {
				this.logger.log(
					`Archive exists for chat room ${chatRoomId}, ${year}-${month}-${day}. Skipping.`,
				);
			...
			this.logger.log(
				`Day ${job.currentDay} completed: ${dayResult.archived} messages archived in ${dayResult.processedBatches} batches`,
			);
		}
	...
	private async processDayArchive(
		...
		const tempFilePath = path.join(
			this.TEMP_DIR,
			`archive-${chatRoomId}-${year}-${month}-${day}.json`,
		);
		...
		try {
			...
			while (offset < messages.length) {
				...
				this.logger.log(
					`Processing batch ${processedBatches + 1}: messages ${offset + 1}-${Math.min(offset + batchSize, messages.length)} of ${messages.length}`,
				);
				...
				fs.writeFileSync(
					tempFilePath,
					JSON.stringify(tempArchive, null, 2),
				);
				...
				this.logger.log(
					`Batch ${processedBatches} completed: ${batch.length} messages added to temp file`,
				);
			...
			this.logger.log(
				`All batches processed. Uploading final archive to cloud storage...`,
			);

			// Upload completed archive to cloud storage
			await this.messagesArchiveService.uploadArchiveFromFile(
				tempFilePath,
				chatRoomId,
				year,
				month,
			...
			this.logger.log(
				`Successfully archived ${messages.length} messages and deleted from database`,
			);

			return { archived: messages.length, processedBatches };
		} catch (error) {
			this.logger.error(
				`Failed to process day archive for ${chatRoomId}, ${year}-${month}-${day}:`,
				error,
			);
	...
	private groupMessagesByDay(messages: any[]): Map<string, any[]> {
		...
		for (const message of messages) {
			if (!message.createdAt) {
				this.logger.warn(
					`Skipping message ${message.id} - no createdAt date`,
				);
				continue;
			}
