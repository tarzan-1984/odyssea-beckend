...
- Duplicate email tracking`,
	...
	@ApiOperation({
		...
		description:
			'ACTIVE drivers (not deactivated, not blocked) with at least one outdated app version. Returns all devices per matching driver. Search matches name, email, externalId (no load id). Default sort: lowest app version first (appVersionSort asc | desc).',
	})
	@ApiQuery({ name: 'page', required: false, type: Number, example: 1 })
	@ApiQuery({ name: 'limit', required: false, type: Number, example: 10 })
	@ApiQuery({
	...
	@ApiOperation({
		...
		description:
			'ACTIVE drivers (not deactivated, not blocked) with two or more devices on one account. Returns all devices per matching driver. Search matches name, email, externalId (no load id). Default sort: lowest app version first (appVersionSort asc | desc).',
	})
	@ApiQuery({ name: 'page', required: false, type: Number, example: 1 })
	@ApiQuery({ name: 'limit', required: false, type: Number, example: 10 })
	@ApiQuery({
