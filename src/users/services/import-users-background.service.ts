...
import { ExternalUser, ExternalUserApiResponse } from '../interfaces/external-user.interface';
...
export class ImportUsersBackgroundService {
  ...
  private readonly EXTERNAL_API_URL = 'https://www.endurance-tms.com/wp-json/tms/v1/users';
  ...
  private async processImportInBackground(jobId: string, startPage: number, per_page: number, search?: string): Promise<void> {
    ...
    try {
      while (true) {
        ...
        const response = await this.fetchExternalUsers(currentPage, per_page, search);
        ...
        // Process users
        const result = await this.processUsersBatch(response.data, duplicateEmails);
      ...
      this.logger.log(`Background import completed for job ${jobId}: ${totalImported} imported, ${totalUpdated} updated, ${totalSkipped} skipped`);
   ...
   */
  private async fetchExternalUsers(page: number, per_page: number, search?: string): Promise<ExternalUserApiResponse> {
    ...
    try {
      ...
      throw new Error(`Failed to fetch users from external API: ${error.message}`);
    }
  }
   ...
   */
  private async processUsersBatch(users: ExternalUser[], duplicateEmails: number[]): Promise<{ imported: number; updated: number; skipped: number }> {
    let imported = 0;
    let updated = 0;
    let skipped = 0;
   ...
   */
  private async processUser(user: ExternalUser, duplicateEmails: number[]): Promise<'imported' | 'updated' | 'skipped'> {
